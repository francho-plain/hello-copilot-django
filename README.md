...
The following files are generated during development and analysis but should not be committed:

**Coverage Reports:**
- `backend/.coverage` - Python coverage data
- `backend/coverage.xml` - Coverage XML report for SonarQube
- `backend/htmlcov/` - HTML coverage reports
- `frontend/coverage/` - React coverage reports

**Analysis Reports:**
- `backend/bandit-report.json` - Security analysis report
- `backend/pylint-report.txt` - Code quality analysis
- `frontend/test-report.xml` - Jest test execution report

**SonarQube Files:**
- `.scannerwork/` - SonarQube scanner temporary files
- `.sonar/` - SonarQube local cache

These files are automatically generated by running tests and quality analysis tools. The `.gitignore` file is configured to exclude these temporary artifacts from version control.
