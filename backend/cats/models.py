...
class Cat(models.Model):
    ...
    name = models.CharField(max_length=100, help_text="The cat's name")

    breed = models.CharField(
        max_length=100, blank=True, help_text="The cat's breed (optional)"
    )

    age = models.IntegerField(blank=True, null=True, help_text="The cat's age in years")

    color = models.CharField(
        max_length=50, blank=True, help_text="The cat's color/pattern"
    )
    ...
    is_neutered = models.BooleanField(
        default=False, help_text="Whether the cat has been neutered/spayed"
    )

    # Adoption information
    owner_name = models.CharField(
        max_length=100, blank=True, help_text="Current owner's name (if adopted)"
    )

    adoption_date = models.DateField(
        blank=True, null=True, help_text="Date when the cat was adopted"
    )

    # Additional information
    description = models.TextField(
        blank=True, help_text="Description of the cat's personality and characteristics"
    )
