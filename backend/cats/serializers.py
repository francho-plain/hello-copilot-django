...
class CatSerializer(serializers.ModelSerializer):
    ...
    def validate_breed(self, value: str) -> str:
        ...
        if value and len(value.strip()) < 2:
            raise serializers.ValidationError(
                "Breed name must be at least 2 characters long"
            )
    ...
    def validate_color(self, value: str) -> str:
        ...
        if value and len(value.strip()) < 2:
            raise serializers.ValidationError(
                "Color description must be at least 2 characters long"
            )
        return value.strip().title() if value else value
    ...
    def validate(self, attrs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if adoption_date and not owner_name:
            raise serializers.ValidationError("Adopted cats must have an owner name")
...
class AdoptCatSerializer(serializers.Serializer):
    ...
    owner_name = serializers.CharField(
        max_length=100, help_text="Name of the new owner"
    )
    adoption_date = serializers.DateField(
        required=False, help_text="Adoption date (defaults to today if not provided)"
    )
